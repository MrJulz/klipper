...
class GCodeParser:
    ...
    def cmd_Tn(self, params):
        ...
        if self.extruder is e:
            return
        old_offset = self.extruder.get_nozzle_offset()
        try:
            ...
            return
        self.extruder = e
        new_offset = self.extruder.get_nozzle_offset()
        for i in range(len(self.base_position)):
            self.base_position[i] += new_offset[i] - old_offset[i]
    ...
    def cmd_M114(self, params):
        ...
        if self.toolhead is None:
            self.cmd_default(params)
            return
        self.respond("X:%.3f Y:%.3f Z:%.3f E:%.3f, Base X:%.3f Y:%.3f Z:%.3f E:%.3f" % (
            self.last_position[0], self.last_position[1],
            self.last_position[2], self.last_position[3],
            self.base_position[0], self.base_position[1],
            self.base_position[2], self.base_position[3]))
